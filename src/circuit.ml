...
let compute_updates aiger =
  ...
  Aiger.LitSet.fold
    (fun _ inp () ->
      let var = BddVariable.of_lit_exn aiger inp in
  ...
  Hashtbl.iter
    (fun l _ ->
      let var = BddVariable.of_lit_exn aiger l in
      let bdd = BddVariable.to_bdd var in
  ...
  Hashtbl.iter
	...
	Hashtbl.add updates (BddVariable.of_lit_exn aiger l) bdd
      with Not_found ->
	Printf.eprintf "warning in Circuit.compute_updates: gate not found\n";
	raise Not_found
    ) aiger.Aiger.latches;
  ...
  Aiger.LitSet.iter
	...
	let bdd_var = BddVariable.of_lit_exn aiger lit in
...
let variables_aiger aiger =
  ...
  let vs =
    Aiger.LitSet.fold
      (fun i inp vs ->
	VariableSet.add (BddVariable.of_lit_exn aiger inp) vs
      ) aiger.Aiger.inputs vs
  in
  let vs =
    Aiger.LitSet.fold
      (fun i out vs ->
	VariableSet.add (BddVariable.of_lit_exn aiger out) vs
      ) aiger.Aiger.outputs vs
  in
  let vs =
    Hashtbl.fold (fun l _ vs ->
      VariableSet.add (BddVariable.of_lit_exn aiger l) vs
...
let of_aiger aiger =
  ...
  let next_variables =
    VariableSet.fold
      (fun x accu -> VariableSet.add (BddVariable.next x) accu)
      variables
      VariableSet.empty
  in
