...
let rec expr_to_bdd expr = match expr with
  ...
  | Boolean.EForall (vl,e) -> 
	 ...
	 (fun accu e -> 
	   ...
	   | _ -> failwith "In ExprToBdd.expr_to_bdd : universal quantification on expressions that are not variables"
	  ) [] vl 
     in
  ...
  | Boolean.EAnd (hd :: tl) -> List.fold_left Cudd.bddAnd (expr_to_bdd hd) (List.map expr_to_bdd tl)
  | Boolean.EAnd [] -> Cudd.bddTrue()
  | Boolean.EOr (hd :: tl) ->  List.fold_left Cudd.bddOr (expr_to_bdd hd) (List.map expr_to_bdd tl)
