...
let synthesize declarations constr =
  ...
  try
    AigerBdd.bdd_to_aiger
      (inputs decl) (registers decl) (outputs decl) (wires decl) bdd
  with (AigerBdd.Unsatisfiable (aig, bdd)) ->
    let input_reg_list =
      AigerBdd.SymbolSet.elements
	(AigerBdd.SymbolSet.union decl.inputs decl.registers)
    in
...
let names_in_boolean_expr expr =
  let rec aux accu = function
    ...
    | Boolean.EAnd tl | Boolean.EOr tl | Boolean.EList tl ->
       List.fold_left aux accu tl
	 
    | Boolean.EExists (tl, e) | Boolean.EForall(tl, e) ->
      (* TODO: Variables in tl should be removed *)
      List.fold_left
        (fun accu -> function
        | Boolean.EVar name -> SymbolSet.remove name accu
        | _ ->
	   failwith ("In Speculog.names_in_boolean_expr: quantification"^
			"over an expression that is not a variables"))
	(aux accu e)
	tl
...
let names_in_expr expr =
  ...
  Array.fold_left
    (fun accu i ->
      SymbolSet.union accu (names_in_boolean_expr i))
    SymbolSet.empty
    arr
...
let types_of_updates updates =
  ...
  let latches,inputs =
    SymbolSet.fold
      (fun s (l,i) ->
	if SymbolSet.mem s write
	then SymbolSet.add s l, i
	else l, SymbolSet.add s i)
      read
      (SymbolSet.empty,SymbolSet.empty)
  in
  let outputs =
    SymbolSet.fold
      (fun s o ->
	if SymbolSet.mem s read
	then o
	else SymbolSet.add s o)
      write
      SymbolSet.empty
  in
